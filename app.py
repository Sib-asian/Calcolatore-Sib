            ...
            if ai_agent and (team_home or team_away):
                with st.spinner("🤖 AI sta analizzando la partita..."):
                    try:
                        ...
                        analysis_prompt = f"Analizza questa partita di calcio con DATI NUMERICI e REALI:\n\n"
                        if team_home:
                            analysis_prompt += f"Squadra Casa: {team_home}\n"
                        if team_away:
                            analysis_prompt += f"Squadra Trasferta: {team_away}\n"
                        analysis_prompt += f"\nDATI MERCATO (USA QUESTI VALORI!):\n"
                        analysis_prompt += f"- Spread Apertura: {spread_opening}\n"
                        analysis_prompt += f"- Total Apertura: {total_opening}\n"
                        analysis_prompt += f"- Spread Corrente: {spread_current}\n"
                        analysis_prompt += f"- Total Corrente: {total_current}\n"
                        analysis_prompt += f"\nISTRUZIONI OBBLIGATORIE:\n"
                        analysis_prompt += f"1. USA calculate_probabilities con questi valori: spread_opening={spread_opening}, total_opening={total_opening}, spread_current={spread_current}, total_current={total_current}\n"
                        analysis_prompt += f"2. USA get_team_news per {team_home if team_home else 'squadra casa'}\n"
                        if team_away:
                            analysis_prompt += f"3. USA get_team_news per {team_away}\n"
                        analysis_prompt += f"\nFORMATO RISPOSTA OBBBLIGATORIO:\n"
                        analysis_prompt += f"## Analisi Numerica\n"
                        analysis_prompt += f"- CITA esplicitamente: Spread Apertura {spread_opening}, Total Apertura {total_opening}\n"
                        analysis_prompt += f"- CITA esplicitamente: Spread Corrente {spread_current}, Total Corrente {total_current}\n"
                        analysis_prompt += f"- USA SEMPRE le probabilità CORRENTI (non quelle di apertura) per le raccomandazioni!\n"
                        analysis_prompt += f"- Mostra probabilità CORRENTI calcolate (1X2, GG/NG, Over/Under) con PERCENTUALI ESATTE\n"
                        analysis_prompt += f"- Analizza movimento: spread da {spread_opening} a {spread_current} (cambio: {spread_current - spread_opening:+.2f})\n"
                        analysis_prompt += f"- Analizza movimento: total da {total_opening} a {total_current} (cambio: {total_current - total_opening:+.2f})\n"
                        analysis_prompt += f"- IMPORTANTE: Quando mostri probabilità 1X2, usa SEMPRE quelle del campo 'current', NON 'opening'!\n"
                        analysis_prompt += f"\n## News e Informazioni (FORMATO OBBLIGATORIO)\n"
                        analysis_prompt += f"- Quando chiami get_team_news, il tool restituisce:\n"
                        analysis_prompt += f"  * 'news': lista di articoli con 'title' e 'snippet'\n"
                        analysis_prompt += f"  * 'injuries': lista di infortuni con 'player' e 'status'\n"
                        analysis_prompt += f"  * 'formations': lista di formazioni (es: ['4-3-3'])\n"
                        analysis_prompt += f"  * 'players_mentioned': lista di giocatori menzionati\n"
                        analysis_prompt += f"- DEVI MOSTRARE questi dati nel formato:\n"
                        analysis_prompt += f"  * News: elenca i TITOLI delle news trovate (max 3)\n"
                        analysis_prompt += f"  * Infortuni: elenca i NOMI dei giocatori infortunati con status (es: 'Lautaro Martinez (out)')\n"
                        analysis_prompt += f"  * Formazioni: mostra le formazioni trovate (es: 'Probabile formazione: 4-3-3')\n"
                        analysis_prompt += f"  * Giocatori chiave: menziona i giocatori più importanti menzionati\n"
                        analysis_prompt += f"- Se get_team_news restituisce array vuoti: scrivi 'Nessuna news/infortunio trovato'\n"
                        analysis_prompt += f"- NON dire solo 'ci sono news disponibili' - DEVI MOSTRARE i dettagli!\n"
                        analysis_prompt += f"- NON inventare informazioni!\n"
                        analysis_prompt += f"\n## Raccomandazioni\n"
                        analysis_prompt += f"- Ogni raccomandazione DEVE citare probabilità specifiche (es: 'Under 2.5 al 58%')\n"
                        analysis_prompt += f"- Ogni raccomandazione DEVE essere basata su spread/total o probabilità calcolate\n"
                        analysis_prompt += f"- NON fare raccomandazioni generiche senza supporto numerico!\n"
                        analysis_prompt += f"\nREGOLE CRITICHE:\n"
                        analysis_prompt += f"- USA SEMPRE calculate_probabilities PRIMA di fare analisi\n"
                        analysis_prompt += f"- CITA SEMPRE i valori numerici (spread {spread_opening}/{spread_current}, total {total_opening}/{total_current})\n"
                        analysis_prompt += f"- NON inventare: se non hai dati reali, dillo chiaramente\n"
                        analysis_prompt += f"- Ogni affermazione deve essere supportata da dati numerici o news reali"
